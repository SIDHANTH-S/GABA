...
export default function PromptComposer() {
  ...
  return (
    <div className="w-full">
      {/* Composer */}
      <div className="relative h-[68px] w-full rounded-t-[20px] bg-[var(--color-subtle)] shadow-[var(--shadow-composer)]">
        <div className="flex size-full items-center gap-[12px] overflow-hidden rounded-t-[20px] px-[20px]">
          <SparkleGlowIcon />
          <input
            value={value}
            onChange={(event) => setValue(event.target.value)}
            placeholder="Ask, automate, or tell me what to do..."
            aria-label="Prompt the assistant"
            className="min-w-px flex-1 bg-transparent text-[10px] font-normal leading-[normal] text-[var(--color-ink)] outline-none placeholder:text-[rgba(46,46,46,0.4)]"
            style={{ fontVariationSettings: '"wdth" 100' }}
          />
          <button type="button" aria-label="Send" className="shrink-0 rounded-full transition duration-150 hover:brightness-105 active:scale-95 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-accent)]/50">
            <SendActionIcon />
          </button>
        </div>
